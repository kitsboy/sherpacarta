...
const checks = [
  ...
  ['curated bundle omits dropped upgrade batches', !read('scripts/bundle-js.mjs').includes("'sc-upgrades-b9.js'") && read('scripts/bundle-js.mjs').includes("'sc-upgrades-b14.js'")],
  ['production bundle does not prefetch Google Fonts', !/fonts\.googleapis\.com|fonts\.gstatic\.com/.test(read('public/sc-bundle.js'))],
  ['production bundle does not self-preload', !/preload["'][^;]{0,80}sc-bundle\.js/.test(read('public/sc-bundle.js'))],
  ['production bundle still owns mobile bottom nav', read('public/sc-bundle.js').includes('mobile-bottom-nav')],
  ['production bundle still owns honest local counts', read('public/sc-bundle.js').includes('local-first') || read('public/sc-bundle.js').includes('SIGN_MODE')],
