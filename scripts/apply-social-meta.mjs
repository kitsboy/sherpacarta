...
const PAGES = {
  'index.html': {
    ...
    // Clean JPEG URL (no query string) — most reliable for X/Twitter cards
    image: '/share.jpg',
    path: '/',
  },
...
function buildBlock(cfg) {
  ...
  // Prefer absolute clean URLs. Optional cache-bust only when cfg.bust is set.
  let image = SITE + cfg.image;
  if (cfg.bust) {
    image += (image.includes('?') ? '&' : '?') + 'v=' + cfg.bust;
  }
  const title = cfg.title;
  const desc = cfg.description;
  const mime = imageType(cfg.image);
  // 1200×630 cards; height still 630 for our assets
  const w = cfg.imageWidth || 1200;
  const h = cfg.imageHeight || 630;
